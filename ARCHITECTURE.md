...
DevPulse is a TypeScript/Vite application with a deliberately small core. The UI is rendered in `src/main.ts`; pure, testable focus-board operations live in `src/domain/focus.ts`. Browser `localStorage` provides the current persistence adapter, while the domain boundary leaves room for other storage implementations.
